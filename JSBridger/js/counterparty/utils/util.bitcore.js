(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        define(['bitcore', '../consts.js', './util.generic.js', 'crypto-js', '../../vendor/mnemonic/mnemonic', './util.bitcoin.js', 'lodash'], function (bitcore, consts, generic, CryptoJS, Mnemonic, btcUtils, _) {
            return factory(bitcore, consts, generic, CryptoJS, Mnemonic, btcUtils, _);
        });
    } else if (typeof module !== 'undefined' && module.exports) {
        module.exports = factory(require('bitcore'), require('../consts.js'), require('./util.generic'), require('crypto-js'), require('../../vendor/mnemonic/mnemonic'), require('./util.bitcoin'), require('lodash'));
    } else {
        // Browser globals
        root.bitcoreUtils = factory(root.bitcore, root.consts, root.generic, root.CryptoJS, root.Mnemonic, root.btcUtils, root._);
    }
}(this, function (bitcore, consts, generic, CryptoJS, Mnemonic, btcUtils, _) {

    'use strict';

// no reason to be used elsewhere
    var NETWORK = consts.USE_TESTNET ? bitcore.networks.testnet : bitcore.networks.livenet;
...
//TODO: rename to be more generic
    ...
    CWBitcore.parseRawTransaction = function (txHex) {
        generic.checkArgType(txHex, "string");
    ...
    CWBitcore.signRawTransaction = function (unsignedHex, cwPrivateKey) {
        generic.checkArgsType(arguments, ["string", "object"]);
    ...
    CWBitcore.extractAddressFromTxOut = function (txout) {
        generic.checkArgType(txout, "object");
    ...
    CWBitcore.extractChangeTxoutValue = function (source, txHex) {
        generic.checkArgsType(arguments, ["string", "string"]);
...
// TODO: check the pubkey instead
    CWBitcore.checkTransactionDest = function (txHex, source, dest) {
        generic.checkArgsType(arguments, ["string", "object", "object"]);
        ...
        for (var i = 0; i < tx.outs.length; i++) {
            ...
            if (!containsSource && !containsDest) {
                ...
                if (tx.outs[i].getValue() > consts.MULTISIG_DUST_SIZE) {
                    return false;
                }
            }
        }
    ...
    var CWHierarchicalKey = function (passphrase, password) {
        generic.checkArgType(passphrase, "string");
        if (password) {
            generic.checkArgType(password, "string");
    ...
    CWHierarchicalKey.prototype.init = function (passphrase) {
        ...
        var words = passphrase.toLowerCase().trim().split(' ');
        var seed = this.wordsToSeed(words);
        this.HierarchicalKey = bitcore.HierarchicalKey.seed(seed, NETWORK.name);
    };
...
//        generic.checkArgType(seed, "string");
...
//        var words = btcUtils.bytesToWordArray(seed);
//        var hash = CryptoJS.HmacSHA512(words, 'Bitcoin seed');
//        hash = btcUtils.wordArrayToBytes(hash);
...
//    };
    ...
    CWHierarchicalKey.prototype.getAddressKey = function (index) {
        generic.checkArgType(index, "number");
...
// priv: private key wif or hex
    var CWPrivateKey = function (priv) {
        generic.checkArgType(priv, "string");
        this.priv = null;
        this.init(priv);
    };
    ...
    CWPrivateKey.prototype.signRawTransaction = function (unsignedHex) {
        generic.checkArgType(unsignedHex, "string");
    ...
    return {
        CWHierarchicalKey: CWHierarchicalKey,
        CWPrivateKey: CWPrivateKey,
        CWBitcore: CWBitcore
    };
}));