...
var tlUtils = require('../common/tl_utils');
var Config = require('../common/config');
var MtpTimeManager = require('./MtpTimeManager');
var q = require('q');
...
var updatesProcessor,
    ...
    offlineInited = false,
    akStopped = false,
//chromeMatches = false, //navigator.userAgent.match(/Chrome\/(\d+(\.\d+)?)/),
//chromeVersion = false, //chromeMatches && parseFloat(chromeMatches[1]) || false,
...
function MtpNetworker(dcID, authKey, serverSalt, options) {
    ...
    this.authKeyUint8 = binUtils.convertToUint8Array(authKey);
    this.authKeyBuffer = binUtils.convertToArrayBuffer(authKey);
    this.authKeyID = binUtils.sha1BytesSync(authKey).slice(-8);
...
MtpNetworker.prototype.wrapApiCall = function (method, params, options) {
    var serializer = new tlUtils.TLSerialization(options);
...
MtpNetworker.prototype.pushMessage = function (message, options) {
    ...
    this.sentMessages[message.msg_id] = _.extend(message, options || {});
    this.sentMessages[message.msg_id].deferred = deferred;
...
MtpNetworker.prototype.pushResend = function (messageID, delay) {
    var value = delay ? utils.tsNow() + delay : 0;
